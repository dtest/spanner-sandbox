...
class TradeLoad(HttpUser):
    ...
    def sellItem(self):
        ...
        # Get a random item
        with self.client.get("/player_items", headers=headers, catch_response=True) as response:
            try:
                ...
                self.client.post("/sell", data=json.dumps(data), headers=headers)
    ...
    def buyItem(self):
        ...
        # Get a random item
        with self.client.get("/open", headers=headers, catch_response=True) as response:
            try:
                orderUUID = response.json()["OrderUUID"]
                buyerUUID = response.json()["BuyerUUID"]

                data = {"orderUUID": orderUUID, "buyer": buyerUUID}
                self.client.put("/buy", data=json.dumps(data), headers=headers)
            except json.JSONDecodeError:
                response.failure("Response could not be decoded as JSON")
            except KeyError:
                response.failure("Response did not contain expected key 'playerUUID'")
